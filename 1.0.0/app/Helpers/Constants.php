        ...
        if (strpos($name, 'get') === 0) {
            ...
            if (property_exists($class, $key)) {
                ...
                if (is_string($value) && !$value) {
                    throw new \App\Exceptions\OutputServerMessageException("`{$key}`不能为空，请确保 SDK 配置已正确初始化");
                }
                return $value;
            }
        }
        if (strpos($name, 'set') === 0) {
            ...
            if (property_exists($class, $key)) {
                if (gettype($value) === gettype(self::$$key)) {
                    ...
                    throw new \App\Exceptions\OutputServerMessageException("Call to method {$class}::{$name}() with invalid arguements");
                }
                return;
            }
        }
        throw new \App\Exceptions\OutputServerMessageException("Call to undefined method {$class}::{$name}()");
    }
}